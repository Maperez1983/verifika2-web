...
export async function POST(request: NextRequest) {
  ...
  const next = sanitizeRelativePath(form.get("next"), "/owner");
  ...
  try {
    ...
    url.searchParams.set("next", "/owner/tratamiento-datos");
  ...
  if (!signerName || !signatureText || !acceptedPrivacy || !acceptedOperations || !acceptedRights) {
    const url = new URL("/owner/tratamiento-datos", origin);
  ...
  try {
    const res = await leadHubFetch("/v1/consents", {
      ...
      body: JSON.stringify({
        persona: "propietario",
        subject_id: consentSubjectForOwner(session),
        listing_ids: session.listingIds,
        name: signerName,
        ...
        source_path: "/owner/tratamiento-datos",
        payload: {
          area: "owner",
          listing_ids: session.listingIds,
